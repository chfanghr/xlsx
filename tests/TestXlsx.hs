...
testXlsx = Xlsx sheets minimalStyles definedNames customProperties DateBase1904
  where
    ...
    foreignDvTestSheet =
        def
            & wsDataValidations .~ foreignValidations
            & wsCells % at (1, 1)
    ...
    sheetView1 =
        def
            & sheetViewRightToLeft ?~ True
            & sheetViewTopLeftCell ?~ CellRef "B5"
    sheetView2 =
        def
            & sheetViewType ?~ SheetViewTypePageBreakPreview
            & sheetViewWorkbookViewId .~ 5
            & sheetViewSelection
                .~ [ def
                        & selectionActiveCell ?~ CellRef "C2"
                        & selectionPane ?~ PaneTypeBottomRight
                   , def
                        & selectionActiveCellId ?~ 1
                        & selectionSqref
                            ?~ SqRef
                                [ CellRef "A3:A10"
                                , CellRef "B1:G3"
...
testCellMap2 =
  ...
  where
    ...
    rich2 =
        def
            & richTextRunText .~ "It's blue!"
            & richTextRunProperties
                ?~ ( def
                        & runPropertiesItalic ?~ True
...
withUnderline _ _ = error "unreachable"
...
testFormatWorkbookResult =
  ...
  where
    ...
    style =
        minimalStyleSheet
            & styleSheetNumFmts .~ M.fromList [(164, "DD.MM.YYYY")]
            & styleSheetCellXfs .~ [cellXf1, cellXf2]
    cellXf1 =
        def
...
testRunCondFormatted = conditionallyFormatted condFmts minimalStyleSheet
  where
    condFmts = flip execState def $ do
        let cfRule1 =
                def
                    & condfmtCondition .~ ContainsBlanks
                    & condfmtDxf % dxfFont % non def % fontUnderline ?~ FontUnderlineSingle
            cfRule2 =
                def
                    & condfmtCondition .~ BeginsWith "foo"
                    & condfmtDxf % dxfFont % non def % fontStrikeThrough ?~ True
            cfRule3 =
                def
                    & condfmtCondition .~ CellIs (OpGreaterThan (Formula "A1"))
