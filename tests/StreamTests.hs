...
import Codec.Xlsx.Writer.Internal.Stream qualified as SW
import Codec.Xlsx.Writer.Stream qualified as SW
...
tests =
    testGroup
        ...
        [ testGroup
            ...
            , testProperty
                "Set of input texts is same as map length"
                sharedStringInputTextsIsSameAsMapLength
            , testProperty
                "Set of input texts is as value set length"
                sharedStringInputTextsIsSameAsValueSetLength
            ]
        , testGroup
            "Reader/Writer"
            [ testCase "Write as stream, see if memory based implementation can read it" $
                readWrite simpleWorkbook
            , testCase "Write as stream, see if memory based implementation can read it" $
                readWrite simpleWorkbookRow
            , testCase "Test a small workbook which has a fullblown sqaure" $
                readWrite smallWorkbook
            , testCase
                [r| Test a big workbook as a full square which caused issues with 
                zipstream. The buffer of zipstream maybe 1kb, this workbook is 
                big enough to be more than that.
                So if this encodes/decodes we know we can handle those sizes.
                In some older version the bytestring got cut off resulting in 
                a corrupt xlsx file.
            |]
