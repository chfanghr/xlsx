...
import Codec.Xlsx.Parser.Stream.HexpatInternal qualified as HexpatInternal
...
parseSharedStrings = \case
    ...
    CharacterData txt -> Nothing <$ (modifying ss_string (<> TB.fromText txt))
    _ -> pure Nothing
...
readSheet (MkSheetIndex sheetId) inner = do
    ...
    let -- TODO: some of the cells are untyped and we need to test whether
...
matchHexpatEvent ev = case ev of
    CharacterData txt ->
        ...
        do
            ...
            when inVal $
                {-# SCC "append_text_buf" #-} (modifying ps_text_buf (<> txt))
            pure Nothing
